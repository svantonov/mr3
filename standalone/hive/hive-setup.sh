...
function hive_setup_print_usage_conf_mode {
    echo " --localthread                  Run MR3 inside Hive as a local thread (default)"
...
function hive_setup_create_output_dir {
    ...
    # append uuidgen because multiple ContainerWorkers may launch at once
    export OUT=$base_dir/hivemr3-$time_stamp-$(uuidgen | awk -F- '{print $1}')
...
-Djava.security.auth.login.config=$BASE_DIR/conf/jgss.conf \
-Djava.security.krb5.conf=$BASE_DIR/conf/krb5.conf \
...
-Dhive.metastore.principal=$HIVE_METASTORE_KERBEROS_PRINCIPAL"
    ...
    # TODO: remove the following block (unnecessary for running Beeline)
    if [ -f $HIVE_SERVER2_SSL_TRUSTSTORE ]; then
      export HADOOP_OPTS="$HADOOP_OPTS \
-Djavax.net.ssl.trustStore=$HIVE_SERVER2_SSL_TRUSTSTORE \
-Djavax.net.ssl.trustStoreType=$HIVE_SERVER2_SSL_TRUSTSTORETYPE \
...
function hive_setup_config_hadoop_classpath {
    ...
    export HADOOP_CLASSPATH=$MR3_CLASSPATH:$TEZ_CLASSPATH:$BASE_DIR/lib/*:$HADOOP_CLASSPATH
}
